...
│       ├── wiggle_appendage.gd    Textured Line2D ribbon + verlet/angular-spring chain (curved rest path)
...
3. **`spriteObject._apply_wiggle_geometry()`** — the single geometry path. Catmull-Rom-smooths `wigglePath` (texture-px control points) into `_wiggleSmooth`, `_rebuild_wiggle_chain()` anchors the ribbon at the path root and calls `appendage.set_geometry(rest_rel, resolution)` (the chain's rest = the path), then `_bake_wiggle_strip()` samples the layer content along the path (perpendicular ±width per column, diffuse + normal `CanvasTexture`) into a straight strip and assigns it as the ribbon texture with a `width_curve` for per-point taper. Re-run on path/width/image change (event-driven, not per-frame).
4. **`spriteObject._update_wiggle(delta)`** — each frame when `wiggleEnabled` (and not path-editing): holds the hidden sprite at identity (so linked children map cleanly), applies the live `wiggleThickness` as a `Line2D.width` scale, `configure()`s the chain from `_wiggle_params()`, and calls `appendage.tick(delta, tick)`. `talkBlink()` copies the sprite's `self_modulate`/`visibility_layer` onto the ribbon so the talk/blink fade still applies.
