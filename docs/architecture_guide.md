...
>
> **BUILD REQUIREMENT (Windows releases):** because `driver.windows="d3d12"`, a Windows export MUST ship the DirectX **Agility SDK `D3D12Core.dll`** (version >= `rendering/rendering_device/d3d12/agility_sdk_version`, currently **618**) next to the exe at `.\x86_64\D3D12Core.dll` (Godot's runtime lookup is `.\<arch>\`, with `.\` as fallback). `export_presets.cfg` has `application/export_d3d12=1`, but Godot's exporter only auto-bundles the SDK when the editor's `export/d3d12/agility_sdk_path` is configured; this build machine bundles `D3D12Core.dll` MANUALLY post-export from the Agility SDK NuGet (`Microsoft.Direct3D.D3D12`, `build/native/bin/x64/D3D12Core.dll`). If a Windows build omits it: `fallback_to_vulkan=true` degrades it to Vulkan (transparency black again) rather than crashing (#104988). Do not skip this step when cutting a Windows release.
