...
> Added: 2026-09-19 — `ui_scenes/spriteList/costume_row.gd` (`CostumeRow`)

The ten costume toggles under the layer list, drawn in code (no image assets).
...
- **Content margins**: 0 on every chip style. Left at default, a `StyleBoxFlat` pads by its border width, and the ring made the worn chip taller than the others
- **No layer selected**: `disabled`, fill `Color(0.18, 0.18, 0.2)`, numerals `SidebarUI.TEXT_DISABLED`
